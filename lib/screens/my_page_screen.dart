            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Row(children: [_StatItem(label: '예약', value: reservations.length.toString(), color: AppColors.primary), _StatItem(label: '차량', value: vehicles.length.toString(), color: AppColors.purple), _StatItem(label: '쿠폰', value: '0', color: AppColors.orange)]),
                ],
              ),
            ),
            if (reservations.isNotEmpty) Container(width: double.infinity, padding: const EdgeInsets.all(24), color: Colors.white, margin: const EdgeInsets.only(top: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('최근 예약', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.textPrimary)), const SizedBox(height: 16), ...reservationService.getRecentReservations().map((reservation) => _ReservationCard(reservation: reservation))])),
            const SizedBox(height: 8),
            _MenuSection(title: '예약 관리', items: [_MenuItem(icon: Icons.calendar_today_outlined, label: '예약 내역', count: reservations.length), _MenuItem(icon: Icons.directions_car_outlined, label: '차량 관리', count: vehicles.length), _MenuItem(icon: Icons.notifications_outlined, label: '알림 설정')]),
            const SizedBox(height: 8),
            ...
            Container(
              ...
              child: InkWell(
                onTap: () async {
                  ...
                  if (confirm == true) { await authService.logout(); vehicleService.clearVehicles(); reservationService.clearReservations(); if (context.mounted) Navigator.popUntil(context, (route) => route.isFirst); }
                },
...
class _ReservationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(reservation.mainOption ?? '예약', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: _getStatusColor().withOpacity(0.1), borderRadius: BorderRadius.circular(4)), child: Text(reservation.isConfirmed ? '확정' : '취소', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _getStatusColor())))],), const SizedBox(height: 8), Text('${reservation.date ?? ''} ${reservation.time ?? ''}', style: const TextStyle(fontSize: 14, color: AppColors.textSecondary))]),
    );
  }
