...
class _PartnerWashBookingScreenState extends State<PartnerWashBookingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('제휴 세차장 예약'), backgroundColor: Colors.white),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            ...dummyWashLocations.map((location) => Padding(padding: const EdgeInsets.only(bottom: 12), child: InkWell(onTap: () => setState(() => _selectedLocationId = location.id), child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: _selectedLocationId == location.id ? AppColors.secondary.withOpacity(0.1) : Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: _selectedLocationId == location.id ? AppColors.secondary : AppColors.border, width: _selectedLocationId == location.id ? 2 : 1)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(location.name, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: _selectedLocationId == location.id ? AppColors.secondary : AppColors.textPrimary)), const SizedBox(height: 8), Row(children: [const Icon(Icons.location_on_outlined, size: 16, color: AppColors.textSecondary), const SizedBox(width: 4), Expanded(child: Text(location.address, style: const TextStyle(fontSize: 14, color: AppColors.textSecondary)))]), const SizedBox(height: 8), Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Row(children: [const Icon(Icons.star, size: 16, color: AppColors.yellow), const SizedBox(width: 4), Text('${location.rating} (${location.reviewCount})', style: const TextStyle(fontSize: 14, color: AppColors.textSecondary))]), Text(location.distance, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.secondary))])]))))),
            const SizedBox(height: 24),
            const Text('세차 종류', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
            const SizedBox(height: 12),
            ...partnerWashServices.map((service) => Padding(padding: const EdgeInsets.only(bottom: 12), child: InkWell(onTap: () => setState(() => _selectedServiceId = service.id), child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: _selectedServiceId == service.id ? AppColors.secondary.withOpacity(0.1) : Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: _selectedServiceId == service.id ? AppColors.secondary : AppColors.border, width: _selectedServiceId == service.id ? 2 : 1)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(service.name, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: _selectedServiceId == service.id ? AppColors.secondary : AppColors.textPrimary)), const SizedBox(height: 4), Text(service.description, style: const TextStyle(fontSize: 14, color: AppColors.textSecondary))]), Text('${service.price.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (m) => '${m[1]},')}원', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: _selectedServiceId == service.id ? AppColors.secondary : AppColors.textPrimary))]))))),
            const SizedBox(height: 24),
