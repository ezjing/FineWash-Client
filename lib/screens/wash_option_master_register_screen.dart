...
/// WashOptionMaster 저장 — option_name, vehicle_type, seq, value1(분), value2(원)
class WashOptionMasterRegisterScreen extends StatefulWidget {
  final int busMstIdx;
  final WashOptionMasterModel? initial;

  const WashOptionMasterRegisterScreen({
    super.key,
  ...
  @override
  State<WashOptionMasterRegisterScreen> createState() =>
      _WashOptionMasterRegisterScreenState();
}

class _WashOptionMasterRegisterScreenState
    extends State<WashOptionMasterRegisterScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.isEditMode ? '옵션(MST) 수정' : '옵션(MST) 등록'),
      ),
      body: Form(
        key: _formKey,
        child: ListView(
